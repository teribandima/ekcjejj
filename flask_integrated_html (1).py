...
<body>
  ...
  <script>
    ...
    function createEducatorCard(educator) {
      ...
      async function renderPanelItems(kind) {
        ...
        items.forEach(item => {
          ...
          thumbRow.className = "item-thumb-row";
          
          const img = document.createElement("img");
          img.className = "mini";
          img.src = item.thumbnail || item.cover_photo || IMG_PH;
          img.alt = item.name;
          img.loading = 'lazy';
          ...
          thumbRow.appendChild(label);
          thumbRow.appendChild(hint);
          itemEl.appendChild(thumbRow);

          const dates = document.createElement("div");
          dates.className = "item-dates";
          
          const startRow = document.createElement("div");
          startRow.className = "item-date-row";
          const startLabel = document.createElement("span");
          startLabel.className = "item-date-label";
          startLabel.textContent = "Starts:";
          const startVal = document.createElement("span");
          startVal.textContent = formatDate(item.starts_at || item.startDate);
          startRow.appendChild(startLabel);
          startRow.appendChild(startVal);
          dates.appendChild(startRow);

          const endRow = document.createElement("div");
          endRow.className = "item-date-row";
          const endLabel = document.createElement("span");
          endLabel.className = "item-date-label";
          endLabel.textContent = "Ends:";
          const endVal = document.createElement("span");
          endVal.textContent = formatDate(item.completed_at || item.ends_at || item.endDate);
          endRow.appendChild(endLabel);
          endRow.appendChild(endVal);
          dates.appendChild(endRow);

          const teachersRow = document.createElement("div");
          teachersRow.className = "item-date-row";
          const teachersLabel = document.createElement("span");
          teachersLabel.className = "item-date-label";
          teachersLabel.textContent = "Teachers:";
          const teachersVal = document.createElement("span");
          teachersVal.style.fontSize = '11px';
          teachersVal.textContent = item.teachers || 'Unknown';
          teachersRow.appendChild(teachersLabel);
          teachersRow.appendChild(teachersVal);
          dates.appendChild(teachersRow);

          itemEl.appendChild(dates);
    ...
    async function renderEducators(page = 1) {
      ...
      if (data.data && data.data.length > 0) {
        ...
        if (page < Math.ceil(data.total / PAGE_SIZE)) {
          ...
          btn.addEventListener("click", () => {
            state.educatorsPage++;
            renderEducators(state.educatorsPage);
          });
    ...
    async function renderSearch(page = 1) {
      ...
      if (data.data && data.data.length > 0) {
        ...
        if (page < Math.ceil(data.total / PAGE_SIZE)) {
          ...
          btn.addEventListener("click", () => {
            state.searchPage++;
            renderSearch(state.searchPage);
          });
          searchLoad.appendChild(btn);
        }
      } else {
        if (page === 1) searchEdu.innerHTML = '<div class="loading-indicator">No results found</div>';
      }
    }
    ...
    document.getElementById("tab-batches").addEventListener("click", () => {
      state.currentTab = 'batches';
      document.getElementById("panel-educators").hidden = true;
      document.getElementById("panel-batches").hidden = false;
      document.getElementById("panel-courses").hidden = true;
      document.getElementById("panel-search").hidden = true;
    });
    ...
    document.getElementById("tab-courses").addEventListener("click", () => {
      ...
      document.getElementById("panel-educators").hidden = true;
      document.getElementById("panel-batches").hidden = true;
      document.getElementById("panel-courses").hidden = false;
      document.getElementById("panel-search").hidden = true;
    });
